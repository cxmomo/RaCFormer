...
void ms_deformable_col2im_cuda_c45(
    const float* grad_col,
    const float* feat_c4,
    const float* feat_c5,
    const int h_c4, const int w_c4,
...
at::Tensor ms_deform_attn_cuda_c45_forward(
    const at::Tensor& feat_c4,  // [B, N, H, W, C]
    const at::Tensor& feat_c5,  // [B, N, H, W, C]
    const at::Tensor& sampling_loc,  // [B, Q, P, 3]
    const at::Tensor& attn_weight  // [B, Q, P, 4]
    ...
    const int batch_size = feat_c4.size(0);
    const int num_views = feat_c4.size(1);
    const int channels = feat_c4.size(4);
    const int num_query = sampling_loc.size(1);
    const int num_point = sampling_loc.size(2);
    AT_ASSERTM(num_point <= MAX_POINT, "num_point exceed limits");
    ...
    auto output = at::zeros({ batch_size, num_query, channels, num_point }, feat_c4.options());
...
std::vector<at::Tensor> ms_deform_attn_cuda_c45_backward(
    const at::Tensor& grad_output,
    const at::Tensor& feat_c4,  // [B, N, H, W, C]
    const at::Tensor& feat_c5,  // [B, N, H, W, C]
    const at::Tensor& sampling_loc,  // [B, Q, P, 3]
    ...
    return {
        grad_value_c4, grad_value_c5, grad_sampling_loc, grad_attn_weight
    };
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("_ms_deform_attn_cuda_c45_forward", &ms_deform_attn_cuda_c45_forward, "pass");
    m.def("_ms_deform_attn_cuda_c45_backward", &ms_deform_attn_cuda_c45_backward, "pass");
