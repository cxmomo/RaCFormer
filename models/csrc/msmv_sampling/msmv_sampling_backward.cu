...
__global__ void ms_deformable_col2im_gpu_kernel_gm_c45(
    const float *grad_col,
    const float *feat_c4,
    const float *feat_c5,
    const int h_c4, const int w_c4,
    ...
    float *grad_attn_weight)
{
    ...
    ms_deformable_col2im_gpu_kernel_gm_c45 <<<GET_BLOCKS(num_kernels, num_threads), num_threads>>>(
        grad_col, feat_c4, feat_c5,
        h_c4, w_c4, h_c5, w_c5,
        data_sampling_loc, data_attn_weight,
        batch_size, channels, num_views, num_query, num_point,
        grad_value_c4, grad_value_c5,
        grad_sampling_loc, grad_attn_weight);

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess)
    {
        printf("error in ms_deformable_col2im_cuda_c45: %s\n", cudaGetErrorString(err));
    }
}
